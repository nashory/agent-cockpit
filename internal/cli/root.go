...
func Execute() error {
	...
	root.AddCommand(&cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Println("Kimi paths:")
			for _, p := range cfg.Paths.Kimi {
				printPath(p)
			}
			fmt.Println("Qwen paths:")
			for _, p := range cfg.Paths.Qwen {
				printPath(p)
			}
			fmt.Println("Codebuff paths:")
			for _, p := range cfg.Paths.Codebuff {
				printPath(p)
			}
			return nil
		},
...
func addFlags(cmd *cobra.Command, opts *options) {
	...
	cmd.PersistentFlags().StringVar(&opts.sources, "source", "", "comma-separated source filter: claude,codex,gemini,opencode,amp,copilot,kimi,qwen,codebuff")
...
func logRoots(cfg config.Config) []string {
	...
	roots = append(roots, cfg.Paths.Kimi...)
